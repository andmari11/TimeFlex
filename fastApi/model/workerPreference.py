...
class WorkerPreference:
    def __init__(self, user_id: int, holidays: Optional[List[datetime.datetime]] = None, preferred_shift_types: Optional[List[int]] = None):
...
def process_worker_preferences(data:json) -> List[WorkerPreference]:
    ...
    for item in data:
        ...
        worker_preference = WorkerPreference(user_id=user_id, holidays=holidays, preferred_shift_types=preferred_shift_types)
