...
N_MIN_HOURS_PER_WORKER = 5
N_MIN_SHIFTS_PER_WORKER = 5
DEBUG = True
...
def optimize(data, logging):
    ...
    for i in range(n_workers):
        worker_takes_shift=[]
        for j in range(n_shifts):
            worker_takes_shift.append(bool2int(Or(all_workers_shifts[i][j], any(
                shifts[j].start.date() == holiday.date() or shifts[j].end.date() == holiday.date()
                for holiday in workers[i].holidays))))
        if(DEBUG):
            ...
            # s.add(addsum(worker_takes_shift) >= N_MIN_SHIFTS_PER_WORKER)

    #comprobamos cuantos horas trabaja cada trabajador
    for i in range(n_workers):
        worker_hours=[]
        for j in range(n_shifts):
            worker_hours.append(If(Or(all_workers_shifts[i][j], any(
                shifts[j].start.date() == holiday.date() or shifts[j].end.date() == holiday.date()
                for holiday in workers[i].holidays)), countHours(shifts[j]), 0))
        if(DEBUG):
            ...
            # s.add(addsum(worker_hours) >= N_MIN_HOURS_PER_WORKER)
