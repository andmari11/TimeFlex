<x-layout :title="'Editar usuario'">
    ...
    <div class=" flex items-center justify-center ">
        <form method="POST" action="/shifts/{{$user->id}}" enctype="multipart/form-data" class="w-full max-w-md bg-white p-8 rounded-lg shadow-md mt-10">
            @csrf
            @method('PATCH')
            <div class="space-y-1 row" >
                ...
                <x-forms.field class="col-12">
                    ...
                    <x-forms.input name="name" placeholder="{{$user->name}}" id="name" value="{{$user->name}}" required />
                    <x-forms.error name="name" />
                </x-forms.field>

                <x-forms.field class="col-12">
                    <x-forms.label for="email">Email</x-forms.label>
                    <x-forms.input name="email" placeholder="{{$user->email}}" id="email" type="email" value="{{$user->email}}" required />
