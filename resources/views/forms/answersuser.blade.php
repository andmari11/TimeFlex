<x-layout :title="'Mis Respuestas a Formularios'">
    <div class="container mx-auto py-10 px-6">
        <!-- Encabezado -->
        <div class="text-center mb-10">
            <h1 class="text-4xl font-extrabold text-gray-900 drop-shadow-lg tracking-tight">Mis Respuestas a Formularios</h1>
            <p class="text-lg text-gray-700 mt-2">Explora tus respuestas organizadas por formulario.</p>
        </div>
        ...
        @else
            <div class="space-y-8">
                @foreach($formularios as $formulario)
                    @php
                        // Filtrar preguntas que tienen respuestas del usuario autenticado
                        $userQuestions = $formulario->questions->filter(function ($question) {
                            return $question->results->filter(function ($result) {
                                return $result->id_user === Auth()->user()->id;
                            })->isNotEmpty();
                        });
                    @endphp
                            ...
                            @foreach($userQuestions as $question)
                                <div class="mt-4">
                                    ...
                                    <ul class="mt-2 space-y-2">
                                        @foreach($question->results->filter(function ($result) {
                                            return $result->id_user === Auth()->user()->id;
                                        }) as $result)
