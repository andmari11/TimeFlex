...
class MenuController extends Controller
{
    public function index(){
        ...
        if (is_object($lastSchedule) && isset($lastSchedule->id)) {
            if(auth()->user()->role === 'admin') {
                $days = MenuController::prepareAllShiftsData();
            } else {
            ...
            if(auth()->user()->role === 'admin') {
                ...
                // Si no hay horarios, mostramos un calendario vacío
    ...
    private function prepareAllShiftsData()
    {
        $shifts = Shift::all();
        ...
        for ($currentDay = $startDate; $currentDay <= $endDate; $currentDay->addDay()) {
            $days->push([
                ...
                'is_today' => $currentDay->isToday(),
                'shifts' => $shifts->filter(fn($shift) => Carbon::parse($shift->start)->isSameDay($currentDay) and
                    (auth()->user()->role === 'admin' or in_array(auth()->user()->id, $shift->users->pluck('id')->toArray())))->take(3)
            ]);
        }
